...
for (let i = 0; i < universities.length; i++) {
  let result = universities[i].toLowerCase().indexOf("uni".toLowerCase()) > -1;
  if (result) {
...
zooAnimals.forEach(zooAnimal => {
  animalNames.push(
    `name: ${zooAnimal.animal_name} , scientific: ${zooAnimal.scientific_name}`
  );
});
...
let lowerCase = [];
lowerCase = zooAnimals.map(zooAnimal => zooAnimal.animal_name.toLowerCase());
...
largerPopulation = zooAnimals.filter(zooAnimal => zooAnimal.population < 5);
